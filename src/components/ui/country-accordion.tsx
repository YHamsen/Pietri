  ...
  return (
    <main className="max-w-4xl w-full mx-auto font-sans antialiased px-4 sm:px-6">
      ...
      <footer className="mt-6 sm:mt-8 flex flex-col items-center gap-5 sm:gap-6">
        ...
        <CtaButton label={ctaText} onClick={onCta} />
      </footer>
    </main>
  );
}
...
function CtaButton({ label, onClick }: { label: string; onClick?: () => void }) {
  return (
    <button
      onClick={onClick}
      className="group flex items-center gap-2 bg-foreground text-background px-7 py-3 rounded-full font-semibold text-sm hover:opacity-90 active:scale-95 transition-all duration-200 shadow-md"
    >
      <span>{label}</span>
      ...
      <div className="p-4 sm:p-5 md:p-6 flex flex-col md:flex-row gap-3 md:gap-10">
        ...
        {hidden.length > 0 && (
          <div className="hidden md:flex md:w-24 justify-end flex-shrink-0 pt-0.5">
            ...
            >
              <span>{isOpen ? 'Réduire' : 'Tout voir'}</span>
              <motion.span
                animate={{ rotate: isOpen ? 45 : 0 }}
                transition={{ duration: 0.25, ease: 'easeInOut' }}
                className="text-base leading-none"
