...
function GlobeLogo({ size = 20 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round">
      ...
      <path d="M9.2 4.5C9.9 3.5 10.9 3 12 3s2.1.5 2.8 1.5"/>
    </svg>
  );
}
...
export default function ConnectAuthPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (mode === 'login') {
      ...
      window.location.href = '/connect/compte';
    } else {
      ...
      setLoading(false);
      if (err) { setError(err.message); return; }
      setSuccess('Vérifie ta messagerie pour confirmer ton compte. Tu peux ensuite te connecter.');
    }
  }
  ...
  return (
    <div style={{ background: '#050505', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'system-ui,-apple-system,sans-serif', color: 'white', padding: '1.5rem' }}>
      ...
      <div style={{ position: 'fixed', top: '-20%', left: '50%', transform: 'translateX(-50%)', width: '70vw', height: '50vh', background: 'radial-gradient(ellipse, rgba(99,102,241,0.07) 0%, transparent 70%)', pointerEvents: 'none' }}/>

      <div style={{ width: '100%', maxWidth: 400, position: 'relative', zIndex: 1 }}>
        {/* Back */}
        <a href="/connect" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', color: 'rgba(255,255,255,0.4)', textDecoration: 'none', fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '2rem', transition: 'color 200ms' }}
          onMouseEnter={e => ((e.currentTarget as HTMLElement).style.color = 'white')}
          onMouseLeave={e => ((e.currentTarget as HTMLElement).style.color = 'rgba(255,255,255,0.4)')}>
          <ArrowLeft size={12} strokeWidth={2.5}/> Retour aux forfaits
        </a>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '2rem' }}>
          ...
          <span style={{ fontFamily: "'Anton',sans-serif", fontSize: '1rem', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
            PIETRI <span style={{ opacity: 0.45 }}>CONNECT</span>
          </span>
        </div>

        {/* Mode toggle */}
        <div style={{ display: 'flex', gap: '0.3rem', marginBottom: '2rem', background: 'rgba(255,255,255,0.04)', borderRadius: 12, padding: '0.3rem', border: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <AnimatePresence mode="wait">
          {success ? (
            <motion.div key="success" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
              style={{ background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: 14, padding: '1.5rem', textAlign: 'center' }}>
              <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'rgba(34,197,94,0.1)', border: '1.5px solid rgba(34,197,94,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1rem' }}>
              ...
              <p style={{ fontSize: '0.72rem', opacity: 0.5, lineHeight: 1.65 }}>{success}</p>
              <button onClick={() => { setSuccess(''); setMode('login'); }} style={{ marginTop: '1.25rem', padding: '0.7rem 1.5rem', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, color: 'white', fontSize: '0.68rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', cursor: 'pointer' }}>
                Se connecter
              </button>
            </motion.div>
          ) : (
            <motion.form key={mode} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
              onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
              ...
              {mode === 'signup' && (
                <div>
                  ...
                  <input type="text" placeholder="Ton prénom" value={nom} onChange={e => setNom(e.target.value)}
                    style={{ ...inp(false), paddingLeft: '1rem' }}/>
                </div>
              )}
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input type="email" required placeholder="ton@email.com" value={email}
                    onChange={e => setEmail(e.target.value)} onFocus={() => setEmailFocus(true)} onBlur={() => setEmailFocus(false)}
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input type={showPwd ? 'text' : 'password'} required placeholder="••••••••" value={password}
                    onChange={e => setPassword(e.target.value)} onFocus={() => setPwdFocus(true)} onBlur={() => setPwdFocus(false)}
              ...
              <button type="submit" disabled={loading}
                style={{ padding: '0.95rem', background: loading ? 'rgba(255,255,255,0.06)' : 'white', color: loading ? 'rgba(255,255,255,0.3)' : '#050505', border: 'none', borderRadius: 12, fontWeight: 700, fontSize: '0.78rem', textTransform: 'uppercase', letterSpacing: '0.14em', cursor: loading ? 'not-allowed' : 'pointer', marginTop: '0.25rem', transition: 'all 200ms' }}>
